...
import { SparseMatrix, Triplet } from "../LinearAlgebra/sparse-matrix";
...
export class Geometry {
	 ...
	 */
	constructor(mesh: Mesh, positions: Vector[], normalizePositions = true) {
		this.mesh = mesh;
		this.positions = {};
	 ...
	 */
	vector(h: Halfedge) {
		let a = this.positions[h!.vertex!.index];
		let b = this.positions[h!.next!.vertex!.index];
	...
	midpoint(e: Edge) {
		let h = e.halfedge;
		let a = this.positions[h!.vertex!.index];
		let b = this.positions[h!.twin!.vertex!.index];
	...
	centroid(f: Face) {
		...
		let a = this.positions[h!.vertex!.index];
		let b = this.positions[h!.next!.vertex!.index];
		let c = this.positions[h!.prev!.vertex!.index];
	 ...
	 */
	laplaceMatrix(vertexIndex: any) {
		let V = this.mesh.vertices.length;
		let T = new Triplet(V, V);
		for (let v of this.mesh.vertices) {
			let i = vertexIndex[v.index];
			let sum = 1e-8;

			for (let h of v.adjacentHalfedges()) {
				let j = vertexIndex[h!.twin!.vertex!.index];
 ...
 */
function normalize(positions: Vector[], vertices: Vertex[], rescale = true) {
