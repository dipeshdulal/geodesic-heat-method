...
export class DenseMatrix {
	...
	constructor(data: any) {
		this.data = data;
		EigenModule.mm.objectList.push(this);
	}
	 ...
	 * @returns {DenseMatrix}
	 */
	static zeros(m: number, n = 1) {
		return new DenseMatrix(new (EigenModule.module() as any).DenseMatrix(m, n));
	}
	 ...
	 * @returns {DenseMatrix}
	 */
	static identity(m: number, n = 1) {
		return new DenseMatrix(EigenModule.module().DenseMatrix.identity(m, n));
	}
	 ...
	 * @returns {DenseMatrix}
	 */
	static ones(m: number, n = 1) {
		return new DenseMatrix(EigenModule.module().DenseMatrix.ones(m, n));
	}
	 ...
	 * @returns {DenseMatrix}
	 */
	static constant(x: number, m: number, n = 1) {
		return new DenseMatrix(EigenModule.module().DenseMatrix.constant(m, n, x));
	}
	 ...
	 * @returns {DenseMatrix}
	 */
	static random(m: number, n = 1) {
		return new DenseMatrix(EigenModule.module().DenseMatrix.random(m, n));
	}
	 ...
	 * @returns {DenseMatrix}
	 */
	subMatrix(r0: number, r1: number, c0 = 0, c1 = 1) {
