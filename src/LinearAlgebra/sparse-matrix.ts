...
export class SparseMatrix {
	 ...
	 */
	static fromTriplet(T: Triplet) {
		return new SparseMatrix(new (EigenModule.module() as any).SparseMatrix(T.data));
	}
	 ...
	 * @returns {SparseMatrix}
	 */
	static identity(m: number, n: number) {
		return new SparseMatrix(EigenModule.module().SparseMatrix.identity(m, n));
	}
	 ...
	 * @returns {SparseMatrix}
	 */
	static diag(d: DenseMatrix) {
		return new SparseMatrix(EigenModule.module().SparseMatrix.diag(d.data));
	}
	 ...
	 * @returns {SparseMatrix}
	 */
	subMatrix(r0: number, r1: number, c0: number, c1: number) {
...
export class Triplet {
	...
	constructor(m: number, n: number) {
		this.data = new (EigenModule.module() as any).Triplet(m, n);
		EigenModule.mm.objectList.push(this);
	}
	 ...
	 * from this triplet.
	 */
	addEntry(x: number, i: number, j: number) {
...
class Cholesky {
	 ...
	 * @returns {DenseMatrix}
	 */
	solvePositiveDefinite(b:DenseMatrix) {
